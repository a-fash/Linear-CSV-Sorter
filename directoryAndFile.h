	...
	if (d) {
		while ((dir = readdir(d)) != NULL)
		{
			if (dir->d_type == DT_REG) //found a file
			{
				...
				//{
					//print child pid to stdout buffer
					//fflush(stdout);
	...
	*/
	...
	switch(result) {
		...
		case -3://header has incorrect column names
			...
			return -1;
			break;
		case -4://a line in the file has an incorrect number of data values
			freeList(head);
			return -1;
			break;
		case -5:
...
void printListToFile(Node *head, char *outputPath)
{
	...
	if(fp == NULL) {
		fprintf(stderr, "ERROR: Could not open output path '%s'\n", outputPath);
		return;
	}
